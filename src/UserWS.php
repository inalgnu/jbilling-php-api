...
class UserWS
{
    public $contact;                         // The primary contact information for this user.
    public $createDateTime;                  // Creation date of this data record.
    public $creditCard;                      // Credit card information for this user. Not required for the user creation process.
    public $currencyId;                      // Contains the currency code for this user. (Refer to Appendix A for acceptable values.)
    public $deleted;                         // If the record has been deleted, this field contains “1”, otherwise it contains “0”. Note that deletion cannot be carried out by simply setting a “1” in this field.
    public $failedAttempts;                  // Number of login attempts that have been failed by this user (i.e., the user has entered the wrong password).
    public $invoiceChild;                    // “true” if this is a sub-account (child of a parent account), but this user will still receive invoices.
    public $isParent;                        // “true” if this record is a “parent” user. A parent user can have sub-accounts (children).
    public $language;                        // Name of the language (i.e. “English”).
    public $languageId;                      // Contains the preferred language code for this user. Refer to Appendix A for acceptable values.
    public $lastLogin;                       // Date of the last login performed by this user.
    public $lastStatusChange;                // Date of the last status change incurred by this user.
    public $mainRoleId;                      // The level of privilege granted to the user when logged into the system. See Appendix A for acceptable values.
    public $parentId;                        // If the user belongs to a parent record, this field contains the identifier of the parent record.
    public $partnerId;                       // Identifier of the partner this user belongs to.
    public $password;                        // Authenticates the user's identity during login. This could be meaningless if the password is encrypted.
    public $role;                            // The name of the role (i.e. “Clerk” or “Customer”).
    public $status;                          // Name of the current status (i.e. “Suspended” or “Active”).
    public $statusId;                        // Current status of the user. See Appendix A for acceptable values.
    public $subscriberStatusId;              // Subscriber status for this user. See Appendix A for acceptable values.
    public $userId;                          // A unique number that identifies the customer
    public $userName;                        // Identifies the user during login
