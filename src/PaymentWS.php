...
class PaymentWS
{
    public $id;              // Unique identifier of the paymentrecord
    public $amount;          // The amount of the payment operation.
    public $balance;         // Balance of this payment. If greater than 0, this payment could pay part of another invoice. If 0, this payment has already been applied to an invoice, lowering the invoice's balance.
    public $createDateTime;  // Date in which this payment record was created.
    public $updateDateTime;  // Date in which this payment record was last updated.
    public $paymentDate;     // Date of the payment.
    public $attempt;         // Number of the attempt to process this payment
    public $deleted;         // “1” if this record has been deleted, “0” otherwise.
    public $methodId;        // Identifier of the payment method. Refer to Appendix A for a list of acceptable values.
    public $resultId;        // Identifier of the result of the payment attempt. Refer to Appendix A for a list of acceptable values.
    public $isRefund;        // Integer “1” if this payment constitutes a refund operation, “0” otherwise.
    public $isPreauth;       // “1” if this payment is a pre-authorization, “0” otherwise.
    public $currencyId;      // Identifier of the currency in which the payment is being made. See Appendix A for a list of acceptable values.
    public $userId;          // Identifier of the user this payment record belongs to.
    public $cheque;          // If this payment is done via check, this property contains information about the cheque, otherwise it contains "null”.
    public $creditCard;      // If this is a credit card payment, this property contains information about the credit card, otherwise it contains "null”.
    public $ach;             // If this is a payment done with Automatic Clearing House (ACH), this property contains the banking information needed.
    public $method;          // Name of the payment method used.
    public $invoiceIds;      // Contains the list of invoices this payment is paying.
    public $paymentId;       // Refund specific field. When a refund is to be issued, this field holds the identifier of the payment that is to be refunded.
    public $authorization;   // Refund specific field. Contains the DTO authorization field for the refund.
