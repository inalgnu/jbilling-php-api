...
class ItemDTOEx
{
    public $currencyId;        // Identifier for the currency in which the item's price is expressed. See Appendix A for a list of acceptable values.
    ...
    public $description;       // The description of this item entityId  Integer Identifier for the entity to which this item belongs.
    public $id;                // A unique number that identifies this record.
    public $number;            // This can be used to identify this item following an external coding system. For example, books can be identified by their ISBN codes.
    public $orderLineTypeId;   // The order type that this item will generate, such a 'taxes', or 'items'.
    public $percentage;        // If this is a percentage item, its rate is specified in this field.
    public $price;             // The price of this item or null if this is a percentage item.
    public $priceManual;       // A flag that indicates if this item will allow manual edition of the price when a purchase order is being placed with the web-based application. A value of 1 will allow manual editing, while a value of 0 will display the price as a read-only field.
    public $prices;            // An item can have many prices, one per currency. This is an array of ItemPriceDTOEx with all the available prices for this item.
    public $promoCode;         // If this item is related to a promotion, this is the code that identifies the promotion.
    public $types;             // A list of type identifiers that indicates to which types[] (categories) this item belongs. An item has to belong to at least one type. Item types are created from the web-based GUI by the billing administrator.
