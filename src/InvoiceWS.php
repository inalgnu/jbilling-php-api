...
class InvoiceWS
{
    public $balance;             // The amount of this invoice that is yet to be paid.
    public $carriedBalance;      // How much of the total belonging to previous unpaid invoices that have been delegated to this one.
    public $createDateTime;      // This is the invoice date, which is assigned to it by the billing process when it is generated.
    public $createTimeStamp;     // A time stamp of when this invoice record was created.
    public $currencyId;          // Identifier of the currency in which the invoice's amounts are being expressed. See Appendix A for a list of all acceptable values.
    public $customerNotes;       // Notes that are entered in a purchase order can be applied to an invoice. If that is the case, this field will have those user notes.
    public $delegatedInvoiceId;  // If this invoice has been included in another invoice (usually for lack of payment), this field will indicate to which invoice it has been delegated.
    ...
    public $dueDate;             // The due date of this invoice. After this date, the invoice should have been paid.
    public $id;                  // A unique number that identifies this record.
    public $inProcessPayment;    // A flag indicating if this invoice will be paid using automated payment (through a payment processor), or if it will be paid externally (for example, with a paper check).
    public $invoiceLines;        // A list of objects representing each of this[] invoice’s lines.
    public $isReview;            // This is an internal value that indicates if this invoice is not a 'real' invoice, but one that belongs to a review process.
    public $lastReminder;        // Date and time of when the latest reminder was issued for this invoice.
    public $number;              // The invoice number, which is assigned to it from a 'preference' field (see the user guide for more information). This is not the ID, which is guaranteed to be unique.
    public $orders;              // A list of the ids of the purchase orders which have been included in this invoice.
    public $overdueStep;         // This marks which step is this invoice in for the penalties (interests) process.
    public $paymentAttempts;     // How many payment attempts have been done by the automated payment process to get this invoice paid.
    public $payments;            // A list of ids of the payments that have been applied to this invoice.
    public $toProcess;           // This is '1' if the invoice will be considered by the billing process as unpaid. Otherwise it is '0' and the invoices is either paid or carried over to another invoice.
    public $total;               // The total amount of this invoice.
    public $userId;              // The customer to whom this invoice belongs.
