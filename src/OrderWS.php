...
class OrderWS
{
    public $activeSince;       // The point in time when this order will start being active, reflecting when the customer will be invoiced for the items included.  A null value indicates that the order was active at creation time (see field createDate).
    public $activeUntil;       // The point in time when this order stops being active. After this date, the order will stop generating new invoices, indicating that the services included in this order should stop being delivered to the customer. A null value would specify an open-ended order. Such order never expires; it is considered on-going and will require explicit cancellation for it to stop generating invoices.
    public $anticipatePeriods; // How many periods in advance the order should invoice for. Leave with a '0' unless you have configured the system to work with anticipated periods.
    public $billingTypeId;     // Indicates if this order is to be paid for before or after the service is provided. Pre-paid orders are invoiced in advance to the customer, while post-paid are only invoiced once the goods or services included in the order have been delivered. “1” means “pre-paid”, while “2” means “post-paid”.
    public $billingTypeStr;    // (read only).This will show the word that represents the billing type. It is ignored when you submit the object.
    public $createDate;        // (read only). A time stamp with the date and time when this order was originally created.
    public $createdBy;         // The id of the user that has created this order.
    public $currencyId;        // Currency code. Refer to Appendix A for a list of acceptable values.
    public $deleted;           // A flag that indicates if this record is logically  deleted in the database. This allows for ‘undo’ of deletions. Valid values are 0 – the record is not deleted 1 – the record is considered deleted.
    public $dfFm;              // Only used for specific Italian business rules.
    public $dueDateUnitId;     // If this order has a specified due date, this will the the units (days, months, years). See Appendix A for valid values.
    public $dueDateValue;      // How many units will be used for the due date.
    public $id;                // A unique number that identifies this record.
    public $lastNotified;      // When the order has expiration notification, this field tells when the last one was sent.
    public $nextBillableDay;   // The date when this order should generate a new invoice. Meaning that until that date (and excluding that date), the customer has been invoiced for the service included in this order.
    public $notes;             // A free text field for your notes.
    public $notesInInvoice;    // “1” if this order's notes will be included in the invoice, or “0” if not.
    public $notificationStep;  // What step has been completed in the order
    public $notify;            // If this order will generate notification as the 'active since' date approaches.
    public $orderLines;        // The order lines belonging to this order. These objects will specify the items included in this order with their prices and quantities. See the OrderLineWS specification for more information.
    public $ownInvoice;        // A flag to indicate if this order should generate an invoice on its own. The default behavior is that many orders can generate one invoice.
    public $period;            // Indicates the periodicity of this order. In other words, how often this order will generate an invoice. Examples of periods are: one time, monthly, weekly, etc. Period codes can be seen in jbilling's User Interface under “Orders -> Periods”.
    public $periodStr;         // (read only). The description of the order period.
    public $statusId;          // An order has to be on status ‘Active’ in order to generate invoices. An order usually starts in active status, and only goes to suspended or finished when the customer fails to make the required payments. The steps and actions taken due to late payments are part of the ageing process. See Appendix A for a list of acceptable order status codes.
    public $userId;            // This order belongs to the user specified by this field.
